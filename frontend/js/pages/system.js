...
async function renderSystem(container) {
    try {
        ...
        const [config, versionInfo, models] = await Promise.all([
            API.getConfig(),
        ...
        container.innerHTML = `
            <div class="card">
                <div class="card-header">
                    ...
                    <button class="btn btn-secondary btn-sm" onclick="renderSystem(document.getElementById('pageContent'))">Refresh</button>
                </div>
                <div class="card-body">
                    <div id="systemAlert"></div>

                    <div class="section-header">
                        <h3 class="section-title">Server Version</h3>
                    </div>
                    ...
                    <div class="stats-grid mb-20">
                        ...
                        <div class="stat-card">
                            ...
                            <div class="stat-value">${(config['usage-statistics-enabled'] ?? config.usage_statistics_enabled) ? '<span class="badge badge-success">Enabled</span>' : '<span class="badge badge-secondary">Disabled</span>'}</div>
                        </div>
                    </div>
...
async function checkForUpdates(currentVersion) {
    ...
    try {
        ...
        if (versionStatus.includes('update available')) {
            alertDiv.innerHTML = `
