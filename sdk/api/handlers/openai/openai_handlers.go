...
import (
	...
	responsesconverter "github.com/giofahreza/AIProxyAPI/internal/translator/openai/openai/responses"
	...
	for {
		...
		case chunk, ok := <-dataChan:
			if !ok {
				...
				setSSEHeaders()
				handlers.WriteSSEFormat(c, "data: [DONE]\n\n")
				flusher.Flush()
				cliCancel(nil)
				return
			}
			...
			handlers.WriteSSEFormat(c, "data: %s\n\n", string(chunk))
			flusher.Flush()
...
func (h *OpenAIAPIHandler) handleCompletionsNonStreamingResponse(c *gin.Context, rawJSON []byte) {
	...
	handlers.WriteSSE(c, completionsResp)
	cliCancel()
}
